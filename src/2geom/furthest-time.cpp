                    ...
                    double from, double to )
{
    ...
    if (c.isConstant()) return to;
    SBasis dd = dot(c - p, dc);
    //std::cout << dd << std::endl;
    std::vector<double> zeros = Geom::roots(dd);

    double furthest = to;
    double max_dist_sq = L2sq(c(to) - p);
    for (double zero : zeros)
    {
        double distsq = L2sq(c(zero) - p);
        if ( max_dist_sq < L2sq(c(zero) - p) )
        {
            furthest = zero;
            max_dist_sq = distsq;
        }
    }
    if ( max_dist_sq < L2sq( c(from) - p ) )
        furthest = to;
    return furthest;
}

...
//      std::cerr << "c[" << i << "] = " << coeff[i] << std::endl;
    ...
    if ( second_sol )
    {
        double prev_mindistsq2 = mindistsq2;
        if ( mindistsq2 > dsq1 )
        {
            result.clear();
            result.push_back(from);
            // this force insert to on equal distance from-to, but this is icorrect because could be not nearest
            mindistsq2 = dsq1;
        }
        else if ( are_near(prev_mindistsq2, dsq1) )
        {
            result.push_back(from);
        }
        if ( mindistsq2 > dsq2 )
        ...
        else if ( are_near(prev_mindistsq2, dsq2) )
        {
            result.push_back(to);
        }
...
std::vector<double> EllipticalArc::allFurthestTimes( Point const& p, double from, double to ) const
{
    ...
    {
        LineSegment seg(pointAt(from), pointAt(to));
        Point np = seg.pointAt( seg.furthestTime(p) );
        if ( are_near(rotationAngle(), M_PI/2)
                || are_near(rotationAngle(), 3*M_PI/2) )
        {
            result = roots(np[Y], Y);
...
//      std::cerr << "c[" << i << "] = " << coeff[i] << std::endl;
    ...
    if ( second_sol )
    {
        double prev_maxdistsq2 = maxdistsq2;
        if ( maxdistsq2 < dsq1 )
        {
            result.clear();
            result.push_back(from);
        ...
        else if ( are_near(prev_maxdistsq2, dsq1) )
        {
            result.push_back(from);
        }
        if ( maxdistsq2 < dsq2 )
        ...
        else if ( are_near(prev_maxdistsq2, dsq2) )
        {
            result.push_back(to);
        }
